...
/***********************************
   DEMBinFile Class Methods
************************************/

//-------------------------------------------------------------------------
//Read the DEM data that falls within a rectangular area defined by the lower left (ll)
//and upper right (ur) coordinates IF THE DEM IS A BIL FILE
//-------------------------------------------------------------------------
int DEMBILReader::ReadRect(char* const chdata,const int minrow, const int maxrow,const int mincol,const int maxcol)
{
   //Convert the coordinates to file r/c to get the bounds to read in
   //These are doubles for future interpolation methods (requiring sub-pixel values) but at present
   //are treated as integers.

   //Check input parameters and throw exception if they are not in correct order
   if ((maxrow < minrow) || (maxcol < mincol))
      throw "Order of elements in DEMBILReader.ReadRect should be llx,lly urx,ury. Min row/col is greater than max row/col.";
   ...
   //methodology.
   unsigned long int numofbytesperline=numsamps*this->GetDataSize();
...
//of nsamps * size of data IF THE DEM IS A BIL FILE
//-------------------------------------------------------------------------
void DEMBILReader::ReadPartOfLine(char* const chdata,const unsigned int lineno,const unsigned int sampleno,const unsigned int nsamps)
{
...
// - This is just a direct copy of the DEMBILReader function since number
// of bands has to be 1 for the DEM file. Done this way to allow future
// update to multiple band files if ever required
//-------------------------------------------------------------------------
int DEMBSQReader::ReadRect(char* const chdata,const int minrow, const int maxrow,const int mincol,const int maxcol)
{
   //Convert the coordinates to file r/c to get the bounds to read in
   //These are doubles for future interpolation methods (requiring sub-pixel values) but at present
   //are treated as integers.
...
//-------------------------------------------------------------------------
void DEMBSQReader::ReadPartOfLine(char* const chdata,const unsigned int lineno,const unsigned int sampleno,const unsigned int nsamps)
{
   if(this->IsGood() == false)
   {
      DEBUGPRINT("ReadPartOfLine has failed...stream not good.") 
